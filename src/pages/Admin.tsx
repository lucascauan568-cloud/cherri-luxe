...
import { fetchProducts, addProduct, updateProduct, deleteProduct, type Product } from "@/lib/products";
import { Plus, Trash2, Pencil, LogIn, X, LogOut } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const handleUpdate = async () => {
    if (!editing) return;
    const success = await updateProduct(editing.id, form);
    if (success) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-[hsl(300,15%,20%)] to-[hsl(320,30%,30%)]">
      <header className="bg-card/90 backdrop-blur-md border-b border-primary/20 px-4 py-4">
        <div className="container flex items-center justify-between">
          ...
          <button
            onClick={() => navigate("/")}
            className="flex items-center gap-2 bg-destructive text-destructive-foreground font-semibold text-sm px-5 py-2.5 rounded-xl hover:bg-destructive/90 transition-all"
          >
      ...
      <div className="container px-4 py-8 max-w-3xl mx-auto space-y-6">
        <div className="bg-primary/10 border border-primary/30 rounded-xl p-3 text-sm text-primary-foreground">
          Esta área é exclusiva do dono do site. Use para inserir produtos, links de afiliados e imagens personalizadas. Os dados são salvos no banco de dados.
        </div>
